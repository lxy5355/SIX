...
Section 2 of this paper provides a framework for SSIMs and a general comparison with parametric and nonparameteric models. Section 3 outlines the identification conditions necessary in SSIMs. Ichimura's (1993) \cite{[6]} and the KS (1993) \cite{[12]} solution will be analyzed in detail in section 4 and 5, respectively. The former section briefly explains the weight function. In section 6, the two models are briefly compared from a theoretical viewpoint. The simulation results from the Monte Carlo experiment are explained in section 7. Finally, in section 8 a real data set example is provided using the pre-processed Gender Recognition by Voice and Speech Analysis dataset.
...
This section provides identification conditions for SSIMs,  summarized in proposition 3.1. Brief intuitive explanations follow each of these conditions. Moreover, it is under these conditions that $\beta_0$ and $ g(\cdot)$ are estimated in Ichimura's (1993) \cite{[6]} and the KS (1993) \cite{[12]} models, which are analyzed in sections 4 and 5 respectively.
...
In this section we analyze the KS (1993) \cite{[12]} semiparametric binary choice model, which is used to estimate model (2) when $Y \in \{0,1\}$.   This method exhibits $n^{-\frac{1}{2}}$ efficiency, asymptotic normality and asymptotic efficiency. 
...
\end{align*}
Based on section 4.2., it follows that the semiparametric maximum-likelihood estimator of $\beta_0$ in the KS (1993) \cite{[12]} model is asymptotically efficient.\footnote{This is only true for ``first order efficiency'', i.e., when $E(X_i|X_i'\beta)$ is linear in $X_i'\beta$.}

\subsection{Bandwidth Selection} % (fold)
\label{sub:Bandwidth Selection}

KS (1993) \cite{[12]} does not discuss the choice of bandwidth, apart from the requirement that $ n^{-\frac{1}{6}} < h_n < n^{-\frac{1}{8}}$. However, similarly to Ichimura's (1993) \cite{[6]} method and H{\"a}rdle et al. (1993) \cite{[18]}, an empirical way of selecting the bandwidth for optimal smoothing of both $g(\cdot)$ and $\beta$ can be conjectured. This is attained by choosing $h_n$ jointly with $\beta$ when maximizing
\begin{equation}
...
For the binary choice case, the KS (1993) \cite{[12]} model seems more adequate than Ichimura's (1993) \cite{[6]}. The KS (1993) \cite{[12]} model is efficient in the sense that it reaches the semiparametric efficiency bound. Maximum-likelihood functions have ``natural'' weighting. On the other hand, Ichimura's (1993) \cite{[6]} model requires a weighting function, and potentially the two-step procedure described in section 4.2., to reach the asymptotic efficiency bound. Thus, much is saved in terms of simplicity and asymptotic efficiency is in general improved by using the KS (1993) \cite{[12]} model for the binary case.

However, restricting analysis to the case of a binary dependent variable is quite stringent. Indeed, this fact gives a clear advantage to Ichimura's (1993) \cite{[6]} model, as his WNLS model allows for continuous outcomes. Thus, it can be applied to a much broader variety of settings than the KS (1993) \cite{[12]}  model.
...
Combining simulation results from table 1 and figure 1, it can be observed that the distributions of all six estimators are skewed to the left, which can be due to finite sample properties. Moreover, the biases are bounded below by - 0.6. 
...
\end{table}
To fully understand the finite sample properties of the estimators, we need to analyze consistency. Indeed, observing table 2, the bias of the estimators presents a decreasing nature for all cases, despite small increases. The KS (1993) \cite{[12]} model exhibits a smaller finite sample bias (ranging in absolute terms from 0.0095 to 0.0302) than Ichimura's (1993) \citep{[6]} model (ranging from 0.0449 to 0.1477) and the logit model (ranging from 0.0303 to 0.2402). Further simulations with increasing number of observations would be desirable, for a more indepth and reliable study of consistency. However, computational issues limit our analysys.

Ultimately, a comparison between our simulation results and Hayfield and Racine's (2008) np-package \cite{[28]} encouraged us to rely on our own implementation. The weight function can however be employed in further simulation studies. 
...
The prediction results and the true values are plotted in figure 2, with grey points representing predicted values and black points true values. A comparison of in-sample and prediction accuracy rate and approximate calculation time are also presented in table 3. The accuracy rate is defined as ratio of correct gender prediction to the total number of data points.


From table 3 and figure 2, we could conclude that the logistic regression would be the preferred method in this case since the improvement in accuracy rate by using semiparametric methods are minimum while the calculation time increases significantly. However, this could be due to the fact that the underlying data fit the logistic distribution better, and thus this comparison should not be generalized to other datasets. 
...
In this paper, we study the theoretical underpinnings and derivations of a special class of semiparametric methods i.e. the single index models, and more specifically, Ichimura's (1993) \cite{[6]} and the KS (1993) \cite{[12]} model. 
We use Monte Carlo experiments to explore the characters of each model. While the finite sample bias decreases for increasing sample size, the KS (1993) \cite{[12]} model exhibits a smaller finite sample bias. Moreover, simulations make computational issues evident. An application on real dataset is presented to further examine the relevance of such methods in real world data analysis. This study has bettered our understanding of semiparametric models and we hope that more applications using such methods could be done in the future. Further effort could be put into the study of better minimization methods or estimation procedures with more relaxed model assumptions. 
...
From table 4 and figure 3, we see that our self-implemented code does not perform significantly worse than the np-package \cite{[28]}. In fact it is even better for the KS (1993) \cite{[12]} method. This could be due to the use of our \textit{a-priori} knowledge of the true value of the coefficients when specifying the grid. We thus conclude that it is an useful and successful practice in constructing our own code for single index model functions.
