...
* In this project, tasks are divided across directories in **src/** in the following way:
    ...
    2. The **model specs** directory includes models characterized by parameters, which are differentiated by sample size and error distribution. Although certain parameters e.g. grid start and grid end for the optimization process, are the same across models, they are not taken out from the model. This is to allow more flexibility and easy implementation for future studies that examine the influence of varying these parameters across models on the efficiency and accuracy of the estimation procedures. 
