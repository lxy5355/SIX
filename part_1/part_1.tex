...
where $\mathbf{1}{(Xi \in A_n)}$ is a trimming function that ensures that the random denominator is positive with high probability, and $w(x_i)$ is an appropriate nonnegative weighting function for possible heteroskedasticity. This might be hard to compute as optimization of a nonlinear potentially multimodal or nonconvex objective function is required.
...
For what follows the trimming set $A_\delta$ is ignored and $w(\cdot)$ is set to 1. Furthermore, assume $\beta - \beta_0 = O(n^{-\frac{1}{2}})$. Then, 
...
\[2(\beta_0 - \beta)\sum_i(g_{i}^{(1)})^2v_iv_i' - 2\frac{1}{n}\sum_i\epsilon_ig_i^{(1)}v_i' = 0 \]
Then, 
\begin{align*}
...
For $g_{i0}^{(1)} = g^{(1)}(X_i'\beta_0)$ and $v_{i0} = X_i - E[X_i'|X_i'\beta_0]$. With this in mind, the Central Limit Theorem (CLT) and Law of Large Numbers (LLN) we obtain the result from Theorem 3.1 if we replace $w(X_i) = 1$ there.
...
The problem of efficient estimation of $\beta_0$ in a single index model with unknown $g$ has been investigated by Hall and Ichimura (1991) and Newey and Stocker (1993). Under certain regularity conditions, for $X \in A_{\delta}$ and $w(x) = \frac{1}{\sigma^2(x)}$, the efficiency bound is
...
This bound is achieved by the semiparametric WNLS estimator if $\sigma^2(X)$ is known or independent of X. When $\sigma^2(X)$ is unknown it can be replaced by a consistent estimator, say $s_{n}^{2}(x)$. If $w(X) = \frac{1}{s_{n}^{2}(x)}$ in the semiparametric WNLS estimator, the asymptotic efficiency bound will be reached. Thus, an asymptotic efficient estimator of $\beta_0$ is obtained even when $\sigma^2(X)$ is unknown. This consistent estimator can be obtained with a two-step procedure. First, estimate $\beta_0$ by minimizing function (9) with $w(x) = 1$. We obtain an estimator that is $n^{\frac{1}{2}}$-consistent and asymptotically normal but inefficient. The second step is a nonparametric estimation of the mean of the squared first-step resilduals conditional on X. More specifically, it uses the estimated weight function $\hat{w}_i(x) = \frac{1}{\hat{\sigma}_{i}^{2}}$, where  $\hat{\sigma}_{i}^{2}$ is defined by Robinson (1987) as 
...
and $\hat{\epsilon}_i = y_i - \hat{g}(x_i'\hat{\beta_0})$, for $\hat{\beta_0}$ obtained on the first step.
...
\end{equation}
Similarly to Ichimura, let $g$ denote the distribution of $\epsilon_i$. Then $G(X_i'\beta) = E(g(X_i'\beta_0)|X_i'\beta)$. Additionaly, assume $\epsilon_i$ is independent of $X_i$. For known $g$, the asymptotically efficient estimator of $\beta_0$ is a maximum likelihood estimator (MLE). The estimated $log-likelihood$ function is

\begin{equation}
\mathcal{L}(\beta) = \sum_i \left\{ (1 - Y_i)ln[ 1 - g(X_i'\beta_0)] + Y_iln[g(X_i'\beta)] \right\},
\end{equation}

where

\begin{enumerate}[label=(\roman*)]
		\item Y is binomial with realizations 1 and 0, X is a vector of exogenous variables, $\beta$ is an unknown parameter vector and $\epsilon$ is a random disturbance;
		\item Observations $\{x_i,y_i\}$ are i.i.d..
\end{enumerate}

The function $\mathcal{L}(\beta)$ is analogous to $S_n(\beta,g)$. Similarly to Ichimura's method, Klein and Spady suggest replacing $g$ with the leave-one-out NW from equation (8). With this in mind, we obtain instead

\begin{equation}
\mathcal{L}_n(\beta) = \sum_i \{ (1 - Y_i)ln[ 1 - \hat{G}_{-i}(X_i'\beta)] +  Y_iln[\hat{G}_{-i}(X_i'\beta)]\}\tau_{i},
\end{equation}

The function $\tau$ is defined in Proposition 4.1.. Furthermore, unlike Ichimura's model, this model is more strict in terms of bandwidth and kernel choice.

\newtheorem{proposition}{Proposition}[section]
\begin{prop}[Trimming function/ kernel and bandwidth choice]
Klein and Spady's model requires:
	\begin{enumerate}[label=(\roman*)]
		\item With $h_N \rightarrow 0$, the trimming function employed to downweight observations has the form
...
where $\epsilon > 0$ and $t$ is to be interpreted as a density estimate.
		\item Kernel $K$ must be of fourth-order by, for example, being bias-reducing. That is,
\[\int z^{2}K(z)dz = 0;\]		
		\item The bandwidth must satisfy the rate $ n^{-\frac{1}{6}} < h < n^{-\frac{1}{8}}$, which is smaller than the optimal rate $n^{-\frac{1}{9}}$ for the fourth-order kernel.
	\end{enumerate}
\end{prop}

With $t$ above replaced by an estimated density, the function downweights observations for which the corresponding densities are small.

For a binary choice model, $Var(Y|X = x) = g(x'\beta_0)[1 - g(x'\beta_0)]$ and $\sigma^2(x)$ depends only on the single index $x'\beta_0$. Then, considering the result shown above
\begin{align*}
g(X_i'\beta_0) - E[g(X_i'\beta_0)|X_i'\beta)] & = g^{(1)}(X_i'\beta)( X_i - E[X_i'|X_i'\beta])(\beta_0 - \beta) + O_p(n^{-1})
\end{align*}
we obtain
\[ \frac{\partial}{\partial \beta} G(X'\beta) = g^{(1)}(X_i'\beta)( X_i - E[X_i'|X_i'\beta]) + o_p(1).\]

By substituting this result in equation (10) and $\Omega_I$ in Theorem 3.1., it becomes clear that the covariance matrix of the asymptotic distribution of the semiparametric WNLS estimator of $\beta_0$ is the same whether the estimator of $g$ is weighted or not. Moreover, the asymptotically efficient bound $\Omega_{SI}$ is reached without using the weight function. 
...
This can be more easily seen by considering a semiparametric WNLS estimator such as the one in equation (9), ignoring for simplicity the trimming function. Differentiate the right-hand side of equation (13) with respect to $\beta$. Then, we obtain
\begin{equation}
\frac{1}{n} \sum_i \frac{Y_i - \hat{G}_{-i}(X_i'\beta)}{\hat{G}_{-i}(X_i'\beta)(1 - \hat{G}_{-i}(X_i'\beta))} \frac{\partial\hat{G}_{-i}(X_i'\beta)}{\partial \beta} = 0
\end{equation} 
...
\begin{align*}
w(x) & = \{ \hat{G}_{-i}(X_i'\beta, \beta)[ 1 - \hat{G}_{-i}(X_i'\beta, \beta)]\}^{-1} = \{ g(X_i'\beta_0)[ 1 - g(X_i'\beta_0)]\}^{-1} \\
	 & + o_p(1) = [Var(Y|X = x)]^{-1} + o_p(1)
\end{align*}


Considering section 3.1., it follows that semiparametric maximum-likelihood estimator of $\beta$ in Klein and Spady's model is asymptotically efficient.\footnote{This is only true for "first order efficiency", i.e., when $E(X_i|X_i'\beta)$ is linear in $X_i'\beta$.}

Under certain assumptions, in particular Proposition 4.1., Klein and Spady (1993) show that $\hat{\beta}_{KS}$ is $n^{-\frac{1}{2}}$ and has asymptotic normal distribution given by

\begin{equation}
\sqrt{n}(\hat{\beta}_{KS} - \beta_0) \stackrel{d}{\rightarrow} N(0,\Omega_{KS}),
\end{equation}
where \[ \Omega_{KS} = \left\{ E\left[\frac{\partial}{\partial \beta}
 G(X'\beta)\frac{\partial}{\partial \beta} G(X'\beta)'\frac{1}{g(X_i'\beta_0)(1 - g(X_i'\beta_0)} \right]\right\}^{-1}. \]
