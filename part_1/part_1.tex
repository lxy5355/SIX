...
Klein and Spady's model seems more approriate than Ichimura's for the binary choice model case. Ichimura's model is heteroskedastic. However, Klein and Spady's model is semiparametrically efficient.  The $log-likelihood$ function has ``natural'' weighting, thus avoiding the problem of heteroskedasticy and the use a weight function.  Consequently, it is not necessary to use the two-step procedure described in section 3.2. to reach the asymptotic efficiency bound. 
