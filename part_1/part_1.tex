...
\[Y = m(X, \epsilon)\]
with smooth $m$ and given sampling,  or  
\[Y = m(X) + \epsilon\]
...
The single index model avoids the curse of dimensionality by reducing the p-dimensional predictor to a univariate single-index. Then $\beta$ achieves the same convergence rate $n^{-\frac{1}{2}}$ as parametric models. Thus, the single index model reaches greater estimation precision than fully nonparametric estimator with multidimensional X and the same precision as a parametric model.
