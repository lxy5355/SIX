...
It is known that $O_p(n^{-2}) = n^{-2}O_p(1) = o_p(1)$ and following the same reasoning $O_p(n^{-1}) = o_p(1)$ and $O_p(n^{-\frac{1}{2}}) = o_p(1)$. Also, considering $E(\epsilon) = 0$, the mean of $\epsilon_i$ is $O_p(n^{-\frac{1}{2}})$. Then, $\frac{1}{n}\sum_i\epsilon_io_p(1) = o_p(1)$. Hence, for $g_{i}^{(1)} = g^{(1)}(X_i'\beta)$ and $v_i = X_i - E[X_i'|X_i'\beta]$:
