...
The function $\tau$ is defined in Proposition 4.1.. Furthermore, unlike Ichimura's approach, this model is more strict in terms of bandwidth and kernel choice.
...
where $\epsilon > 0$ and $t$ is to be interpreted as a density estimate.
		\item The kernel $K$ must be of fourth-order.		
		\item The bandwidth must satisfy the rate $ n^{-\frac{1}{6}} < h < n^{-\frac{1}{8}}$.
	\end{enumerate}
\end{prop}

By replacing $t$ by an estimated density in part $(i)$, observations for which the corresponding densities are small are downweighted.
Part $(ii)$ can be satisfied by, for example,  having a bias-reducing kernel. That is,
\[\int z^{2}K(z)dz = 0.\]
Furthermore, the rate required for $h$ in $(iii)$ is smaller than the optimal rate $n^{-\frac{1}{9}}$ for the fourth-order kernel.


Under certain assumptions, in particular Proposition 4.1., Theorem 4.1. gives as the following result:
...
It now becomes necessary to elaborate on the last stament of Theorem 4.1. For a binary choice model, $Var(Y|X = x) = g(x'\beta_0)[1 - g(x'\beta_0)]$ and $\sigma^2(x)$ depends only on the single index $x'\beta_0$. Then, considering the result shown above
\begin{align*}
