...
In this section we analyze Ichimura's (1993) estimation method for semiparametric models. Were $g(\cdot)$ to be known and a nonlinear least squares method could be used to estimate $\beta_0$ y minimizing:
...
where $\hat{p}_{-i}(X_i'\beta) = (nh)^{-1}\sum_{j=1,j &\neq i}^{n}  Y_jK(\frac{X_j'\beta - X_i'\beta}{h})}$.  The denominator $\hat{p}(X_i'\beta)$ is random and it becomes necessary to trim out small values. Let $p(x'\beta)$ denote de probability density function (PDF) of $X_i'\beta$ and $A_\delta$ and $A_n$ be the following sets:
...
Then for $x \in A_\delta$ the denominator does not get too close to zero. The set $A_n$ where $\norm{\cdot}$ is a Euclidean norm is larger than $A_\delta$ but as $ n \rightarrow \infty $, $h \rightarrow 0$, $A_n$ shrinks to $A_\delta$. With this in mind choose $\beta$ by minimizing the objective function:

\begin{equation}
S_n(\beta) = \sum_{i=1}^{n}  [Yi - \hat{G}_{-i}(X_i'\beta)]^2w(xi)\mathbf{1}{(Xi \in A_n)}
\end{equation}
...
\[ \sqrt{n}(\hat{\beta} - \beta_0}) \rightarrow N(0,\Omega_I) \] in ditribution,

where $\Omega_I = V^{-1}\Sigma V^{-1}$, and where
...
where $S(\beta) = n^{-1} \sum_i [Y_i - G(X_i'\beta)]^2$, where

\[ T_n = \sum_{i=1}^{n}[\hat{G}_{-i}(X_i'\beta_0) - g(X_i'\beta_0)]^2 \]

is a term that is independent of $\beta$. In section 1 of the appendix we show that $S(\beta) = O(1)$. Thus minimizing $S_n(\beta)$ with respect to $\beta$ is asymptotically equivalent to minimizing $S(\beta)$. There we show that $\bar{\beta}$ satisfies the followinf first order condition:
...
\[  W_0 = \sum_{i=1}^{n} u_i[f^{(1)}(X_i'\beta_0)]^2[X_i - E(X_i|X_i'\bet_0][X_i - E(X_i|X_i'\beta_0)]' \]

and

\[ V_0 = \sum_{i=1}^{n} u_if^{(1)}(X_i'\beta_0)[X_i - E(X_i|X_i'\beta_0)\].
