...
Klein and Spady (1993) do not discuss how to choose the bandwidth, appart from the requirement that $ n^{-\frac{1}{6}} < h_n < n^{-\frac{1}{8}}$. It can be conjectured, following H{\"a}rdle et al. (1993) that it can be chosen jointly with $\beta$ when minimizing
\begin{equation}
...
Klein and Spady's model seems more approriate than Ichimura's for the binary choice model case. Ichimura's model is heteroskedastic. However, for Klein and Spady's model the $log-likelihood$ function has ``natural'' weighting, thus avoiding the problem of heteroskedasticy. Therefore, it does not become necessary to use a weight function and consequently it is also not necessary to use the two-step procedure described in section 3.2. to reach the asymptotic efficiency bound. 
