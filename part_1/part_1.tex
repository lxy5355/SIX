...
For the first case where $X_2 = 0$ the function g is identified on $\big[0,1\big]$. However, for $\beta_2 > 1$ the support of $ X_1 + \beta_2$ is disjoint from $\big[0,1\big]$ and $\beta_2$ is an intercept in the model for $E(Y|(x_1,1))$. Thus, it is not possible to identify $\beta_2$. However, for $\beta_2 < 1$ the support of $X_1$ and $X_1 + \beta_2$ overlap. In particular, for $\beta_2$ positive and the subset $\{X: X_1 \in [ 0, 1 - \beta_2] \wedge X_2 = 1\}$, $g(x_1 + \beta_2) = g(v)$ for some $v \in [0,1]$. \footnote{For $\beta_2$ negative one can consider $\{X: X_1 \in [ - \beta_2, 1 + \beta_2] \wedge X_2 = 1\}$} Then, $g(v)$ can be identified for $v \in [\beta_2, 1]$ by observations of $X_1$ for which $X_2 = 0$. Therefore, $\beta_2$ can be identified by solving
...
\end{equation}
 by the kernel method. Ichimura (1993) proposes modifications of the usual kernel estimation to estimate $G(X_i'\beta)$. Observation $i$ is excluded from the calculation of $G(X_i'\beta)$. To understand why this is necessary, suppose a relatively small bandwidth is used. Then the estimates will be quite close to the observation values. Thus, equation 6 will be aproximately zero. By leaving one observation out, this problem is countered. Thus a leave-one-out Nadaraya-Watson (NW) kernel estimator is obtained:
...
where $\hat{p}_{-i}(X_i'\beta) = \frac{1}{nh}\sum_{j=1,j \neq i}^{n}K(\frac{X_j'\beta - X_i'\beta}{h})$.  The denominator $\hat{p}(X_i'\beta)$ is random and it becomes necessary to trim out small values. Let $p(x'\beta)$ denote the probability density function (PDF) of $X_i'\beta$ and $A_\delta$ and $A_n$ be the sets
...
\[\int z^{2}K(z)dz = 0.\]
Furthermore, the bandwidth must satisfy the rate $ n^{-\frac{1}{6}} < h_n < n^{-\frac{1}{8}}$. Given these restrictions, we obtain Theorem 4.1.
...
\begin{align*}
g(X_i'\beta_0) - E_A[g(X_i'\beta_0)|X_i'\beta)] & = g^{(1)}(X_i'\beta)( X_i - E_A[X_i'|X_i'\beta])(\beta_0 - \hat{\beta}) \\
											   & + O(n^{-1})
\end{align*}
where $E_A(X_i|v) = E(X_i|x_A'\beta_0 = v)$ with $x_A$ having the distribution of $X_i$. Minimizing in order to $\hat{\beta}$ and ignoring terms independent of $\beta$ and $O(n^{-1})$:
\begin{align*}
...
This can be more easily understood by considering a semiparametric WNLS estimation of $\beta_0$ such as with Ichimura's method. Differentiate the right-hand side of equation (13) with respect to $\hat{\beta}_n$. Then, we obtain
\begin{equation}
\frac{1}{n} \sum_{i=1}^n \tau_i \frac{Y_i - \hat{G}_{-i}(X_i'\hat{\beta}_n)}{\hat{G}_{-i}(X_i'\hat{\beta}_n)(1 - \hat{G}_{-i}(X_i'\hat{\beta}_n))} \frac{\partial\hat{G}_{-i}(X_i'\hat{\beta}_n)}{\partial \hat{\beta}_n} = 0
\end{equation} 
...
\begin{align*}
w(x) & = \{ \hat{G}_{-i}(x'\hat{\beta}_n)[ 1 - \hat{G}_{-i}(x'\hat{\beta}_n)]\}^{-1} = \{ G(x'\hat{\beta}_n)[ 1 - G(x'\hat{\beta}_n)]\}^{-1} + o_p(1) \\
...
Klein and Spady (1993) do not discuss how to choose the bandwidth, apart from the requirement that $ n^{-\frac{1}{6}} < h_n < n^{-\frac{1}{8}}$. It can be conjectured, following H{\"a}rdle et al. (1993) that it can be chosen jointly with $\beta$ when minimizing
\begin{equation}
...
Klein and Spady's model seems more approriate than Ichimura's for the binary choice model case. Ichimura's model uses a weight function as described in section 3.2. to correct for heteroskedasticity. However, Klein and Spady's model is semiparametrically efficient.  The $log-likelihood$ function has ``natural'' weighting, thus avoiding the problem of heteroskedasticity and the use a weight function.  Consequently, a clearer and more easily applied model is obtained that avoids the use of the two-step procedure described in section 3.2. to reach the asymptotic efficiency bound. 
